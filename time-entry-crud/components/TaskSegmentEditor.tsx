...
import { TaskSegment, validateSegments } from "../lib/task-segments";

interface Props {
  segments: TaskSegment[];
  onChange: (segments: TaskSegment[]) => void;
  /** List of choices for the segment label (department, activity, sub-task...). */
  departments: string[];
  passStart: string;
  passEnd: string;
  /** Header label. Default: "Department logging" */
  title?: string;
}

export default function TaskSegmentEditor({
  segments,
  onChange,
  departments,
  passStart,
  passEnd,
  title = "Department logging",
}: Props) {
    ...
    <div className="mt-4">
      ...
      {open && (
        <div className="mt-3 border border-gray-200 rounded-lg p-4 bg-gray-50">
          ...
          {segments.length === 0 ? (
            <p className="text-sm text-gray-500 mb-3">No segments added.</p>
          ) : (
